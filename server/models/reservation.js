...
const dbModel = {
  ...
  readCalendar: async (p) => {
    ...
    let sql = `SELECT id, reserver_id, space, state, time_from, time_to, content FROM reservation WHERE ((MONTH(time_from) = MONTH(?) AND YEAR(time_from) = YEAR(?)) OR (MONTH(time_to) = MONTH(?) AND YEAR(time_to) = YEAR(?))) AND state!='rejected' `;
