...
const dbModel = {
  ...
  readCalendar: async (p) => {
    ...
    let sql = `SELECT id, space, state, time_from, time_to, content FROM reservation WHERE ((MONTH(time_from) = MONTH(?) AND YEAR(time_from) = YEAR(?)) OR JSON_EXTRACT(content, '$.recurrenceRule') IS NOT NULL) AND state!='rejected' `;
